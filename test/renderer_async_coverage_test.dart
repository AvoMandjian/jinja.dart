...
void main() {
  ...
  group('AsyncRenderer Coverage Extensions', () {
    test('visitConcat (async)', () async {
      ...
      final result = await template.renderAsync({'async_val': Future.value('c')});
      expect(result, equals('abc'));
    });

    test('visitCondition (async)', () async {
      final template = env.fromString('{{ "yes" if async_bool else "no" }}');
      expect(await template.renderAsync({'async_bool': Future.value(true)}), equals('yes'));
      expect(await template.renderAsync({'async_bool': Future.value(false)}), equals('no'));
    });
    ...
    test('visitDict (async)', () async {
      ...
      final template = env.fromString('{% set d = {async_key: async_val} %}{{ d["a"] }}');
    ...
    test('visitFor with test and orElse (async)', () async {
      final template = env.fromString('{% for i in items if i > 1 %}{{ i }}{% else %}none{% endfor %}');
      
      // All filtered out
      expect(await template.renderAsync({'items': [0, 1]}), equals('none'));
      // Some pass
      expect(await template.renderAsync({'items': [1, 2, 3]}), equals('23'));
      // Empty
    ...
    test('visitTemplateNode super() and self (async)', () async {
      final env2 = Environment(
        loader: MapLoader({
          ...
          'child': '{% extends "parent" %}{% block b %}child {{ super() }}{% endblock %}',
        }, globalJinjaData: {}),
      );
    ...
    test('visitLogical short-circuit (async)', () async {
      final template = env.fromString('{{ true or async_val }} {{ false and async_val }}');
      final result = await template.renderAsync({'async_val': Future.value('not_called')});
