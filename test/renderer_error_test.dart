...
void main() {
  group('Renderer specific error paths', () {
    test('error wrapper in condition', () {
      final envThrow = Environment(getAttribute: (attr, obj, {node, source}) => throw Exception('error'));
      final t = envThrow.fromString('{% if obj.foo %}A{% endif %}');
      expect(() => t.render({'obj': _ThrowingObj()}), throwsA(isA<TemplateErrorWrapper>()));
    });
    ...
    test('error wrapper in name resolution', () {
      final envThrow = Environment(undefined: (name, [tmpl]) => throw Exception('error'));
    ...
    test('error wrapper in for loop', () {
      ...
      expect(() => t.render({'bad': _ThrowingObj()}), throwsA(isA<TemplateErrorWrapper>()));
    });
  });
}
