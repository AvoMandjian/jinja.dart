...
base class StringSinkRenderer extends Visitor<StringSinkRenderContext, Object?> {
  ...
  MacroFunction getMacroFunction(
    ...
    Object? macro(List<Object?> positional, Map<Object?, Object?> named) {
      print('DEBUG: macro ${node.name} called with positional=$positional, named=$named');
          ...
          if (index < positional.length) {
            ...
            if (remaining.remove(key)) {
              ...
              if (defaultValue is Name) {
                ...
                if (defaultValueValue == null && !named.containsKey(defaultValue.name) && !named.containsKey(Symbol(defaultValue.name))) {
                  // Not provided in named arguments. Let's resolve it in the derived context,
                  // which contains already parsed positional arguments and globals.
                  defaultValueValue = defaultValue.accept(this, derived);
                }
      ...
      if (buffer is _AsyncCollectingSink) {
        ...
        // Execute the macro body first to see if it generates any Futures
        node.body.accept(this, derived);

        // If no Futures were generated, return the result synchronously!
        if (asyncBuffer.futuresCount == 0) {
          return SafeString(asyncBuffer.syncContent);
        }

        // Otherwise, wrap the async resolution in a Future
  ...
  Parameters visitCalling(Calling node, StringSinkRenderContext context) {
    ...
    for (var argument in node.arguments) {
      print('DEBUG AST StringSink: argument=$argument (type: ${argument.runtimeType})');
      var res = argument.accept(this, context);
      print('DEBUG StringSink visitCalling: arg=${argument.runtimeType} => $res');
      positional.add(res);
    }
  ...
  Object? visitConcat(Concat node, StringSinkRenderContext context) {
    ...
    if (hasFuture) {
      return Future.wait(
        ...
        final result = resolvedValues.join();
        context.environment.debugJinja(
  ...
  Object? visitSlice(Slice node, StringSinkRenderContext context) {
    try {
      ...
      if (value is String && start is int && stop is int?) {
        if (start < 0 || (stop != null && stop < start)) {
          ...
          throw TemplateRuntimeError(
            ...
            nodeValue: node,
            operationValue: 'Slicing string',
            suggestionsValue: suggestions,
            templatePathValue: context.template,
          );
        }
        if (start >= value.length) {
          return ''; // In Python, slicing past the end of a string returns an empty string
        }

        var actualStop = stop;
        if (actualStop != null && actualStop > value.length) {
          actualStop = value.length;
        }

        return value.substring(start, actualStop);
      }
      ...
      final suggestions = <String>[
        'Slice operation only works on lists and strings',
        'Value type: ${value.runtimeType}, expected List or String',
        'Start type: ${start.runtimeType}, expected int',
        if (stop != null) 'Stop type: ${stop.runtimeType}, expected int',
      ];
      throw TemplateRuntimeError(
        'Invalid slice operation: value is ${value.runtimeType}, not List or String.',
        nodeValue: node,
...
class AsyncStringSinkRenderer extends Visitor<AsyncRenderContext, Future<Object?>> {
  ...
  Future<Parameters> visitCalling(
    ...
    var positional = <Object?>[];
    for (var argument in node.arguments) {
      print('DEBUG AST: argument=$argument (type: ${argument.runtimeType})');
      var res = await argument.accept(this, context);
      print('DEBUG Async visitCalling: arg=${argument.runtimeType} => ${argument.toSource()} evaluated to $res');
      positional.add(res);
    }
    ...
    print('DEBUG Async visitCalling: named = $named');
...
class _AsyncCollectingSink implements StringSink {
  ...
  /// Get the synchronous content if there are no Futures
  String get syncContent => _buffer.toString();
