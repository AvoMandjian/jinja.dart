...
void main() {
  group('Environment', () {
    ...
    test('hashCode and operator ==', () {
      final env1 = Environment(blockStart: '{%', blockEnd: '%}');
      final env2 = Environment(blockStart: '{%', blockEnd: '%}');
    ...
    test('callCommon ContextFilter without context', () {
      ...
      expect(() => env.callCommon(filter, [], {}, null), throwsA(isA<TemplateRuntimeError>().having((e) => e.message, 'message', contains('without context'))));
    });
    ...
    test('callCommon Invalid callable', () {
      final env = Environment();
      expect(() => env.callCommon(42, [], {}, null), throwsA(isA<TemplateRuntimeError>().having((e) => e.message, 'message', contains('Invalid callable'))));
    });

    test('callFilter ContextFilter without context', () {
      final env = Environment(filters: {'ctx': ContextFilter((c) => 'ok')});
      expect(() => env.callFilter('ctx', [], {}, null), throwsA(isA<TemplateRuntimeError>().having((e) => e.message, 'message', contains('without context'))));
    });

    test('callFilter async arguments', () async {
      final env = Environment(filters: {'id': (x) => x});
      final result = await env.callFilter('id', [Future.value(42)], {}, null);
      expect(result, equals(42));
    });

    test('callFilter error wrapping', () {
      final env = Environment(filters: {'fail': () => throw Exception('oops')});
      expect(() => env.callFilter('fail', [], {}, null), throwsA(isA<TemplateErrorWrapper>()));
    });

    test('callTest async arguments', () async {
      final env = Environment();
      // 'defined' test
      final result = await env.callTest('defined', [Future.value(42)], {}, null);
