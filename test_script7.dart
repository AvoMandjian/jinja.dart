...
void main() {
  ...
  final template = env.fromString(script);
  final output = (template.body as TemplateNode).body as Output;
  for (var i = 0; i < output.nodes.length; i++) {
    var node = output.nodes[i];
    if (node is Interpolation) {
       var call = node.value as Call;
       print("Interpolation " + i.toString() + ": args=" + call.calling.arguments.map((e) => e.runtimeType).toString());
    }
  }
}
