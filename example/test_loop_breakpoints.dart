...
End''';
  ...
  await runDebugTest(template, {
    'range': (int start, [int? stop, int step = 1]) => 
      stop == null ? Iterable<int>.generate(start) : 
      Iterable<int>.generate((stop - start) ~/ step, (i) => start + i * step)
  }, [3]); // Breakpoint on line 3 (Item {{ i }})
}
...
End''';
  ...
  await runDebugTest(template, {
    'range': (int start, [int? stop, int step = 1]) => 
      stop == null ? Iterable<int>.generate(start) : 
      Iterable<int>.generate((stop - start) ~/ step, (i) => start + i * step)
  }, [3, 5]); // Breakpoints on lines 3 and 5
}
...
Final: {{ total }}''';
  ...
  await runDebugTest(template, {
    'items': [10, 20, 30],
    'total': 0,
  }, [4, 5, 7]); // Breakpoints on Value, Running total, and Final
}
