...
void main() {
  ...
  group('StringSinkRenderer Coverage Extensions', () {
    test('visitCompare all operators', () {
      ...
      expect(
          renderer.visitCompare(
              Compare(
                  value: Constant(value: 1),
                  operands: [(CompareOperator.notEqual, Constant(value: 2))]),
              context),
          isTrue);
      // Less Than
      expect(
          renderer.visitCompare(
              Compare(
                  value: Constant(value: 1),
                  operands: [(CompareOperator.lessThan, Constant(value: 2))]),
              context),
          isTrue);
      // Less Than Or Equal
      expect(
          renderer.visitCompare(
              Compare(value: Constant(value: 1), operands: [
                (CompareOperator.lessThanOrEqual, Constant(value: 1))
              ]),
              context),
          isTrue);
      // Greater Than
      expect(
          renderer.visitCompare(
              Compare(value: Constant(value: 2), operands: [
                (CompareOperator.greaterThan, Constant(value: 1))
              ]),
              context),
          isTrue);
      ...
      expect(
          renderer.visitCompare(
              Compare(value: Constant(value: 2), operands: [
                (CompareOperator.greaterThanOrEqual, Constant(value: 2))
              ]),
              context),
          isTrue);
      // Contains
    ...
    test('visitUnary all operators', () {
      ...
      expect(
          renderer.visitUnary(
              Unary(operator: UnaryOperator.plus, value: Constant(value: 1)),
              context),
          equals(1));
      expect(
          renderer.visitUnary(
              Unary(operator: UnaryOperator.minus, value: Constant(value: 1)),
              context),
          equals(-1));
      expect(
          renderer.visitUnary(
              Unary(operator: UnaryOperator.not, value: Constant(value: false)),
              context),
          isTrue);
    });
    ...
    test('visitAssignBlock with filters', () {
      ...
      final nodeWithFilter = AssignBlock(
        ...
        filters: [
          Filter(name: 'upper', calling: Calling(arguments: [], keywords: []))
        ],
        body: Data(data: 'abc'),
      );
    ...
    test('visitAutoEscape', () {
      ...
      expect(
          sink.toString(),
          equals(
              '<b>')); // Not escaped because autoEscape was disabled by block
    });

    test('visitDebug', () {
      final sink = StringBuffer();
      final context =
          StringSinkRenderContext(env, sink, data: {'a': 1, 'b': 2});
    ...
    test('visitCondition (ternary)', () {
      ...
      // True case
      final nodeTrue = Condition(
          test: Constant(value: true),
          trueValue: Constant(value: 'yes'),
          falseValue: Constant(value: 'no'));
      expect(renderer.visitCondition(nodeTrue, context), equals('yes'));

      // False case
      final nodeFalse = Condition(
      ...
      final nodeNoFalse = Condition(
          test: Constant(value: false), trueValue: Constant(value: 'yes'));
